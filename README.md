...
- Live Realtime DB subscriptions for picks and history feeds.
- Firebase email/password + Google authentication helper methods.
- Paystack integration with server verification callback and duplicate-reference guard.
- Firebase Realtime Database rules in `rules.json`.

## Project Structure
- `src/app` - routes and API handlers
- `src/components` - UI components (tables, row paywall, badges, navbar)
- `src/hooks` - client hooks for auth/profile and live matches
